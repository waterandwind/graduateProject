        ...
        log.error("请求---->>>>>>>>>"+url);
            if (url.contains("login") || url.contains("swagger") || url.contains("logout") || url.contains("v2")|| url.contains("upload")|| url.contains("imgShow")|| url.contains("mobile")) {
                ...
                if (token != null) {
                    ...
                    boolean hasRight = isHavaRight(url,reAccount);
                    if (reAccount != null && account.equals(reAccount)&&hasRight) {
...
//                        log.error("token失效");
                            ...
                            if (!hasRight){
                                response1 = Response.builder().msg("权限不足，请联系管理员").code(ResponseStatus.RIGHT_NOT_ENOUGHT).build();
                            }else {
                              response1 = Response.builder().msg("token失效").code(ResponseStatus.INVALID_TOKEN).build();
                            }
    ...
    public boolean isHavaRight(String url,String account){
        List<String> rightList =(List<String>) redis.opsForList().range("r"+account,0,-1);
//        log.error("");
